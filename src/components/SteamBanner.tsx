...
export default function SteamBanner({ appid, name }: { appid: number, name: string }) {
  const [src, setSrc] = useState(`https://shared.fastly.steamstatic.com/store_item_assets/steam/apps/${appid}/header.jpg`);
  ...
  const handleError = () => {
    if (retry === 0) {
      ...
      setSrc(""); // Final fallback to technical pattern
    }
  };
